...
tf.flags.DEFINE_integer("num_filters", 128) # The number of fliter size
...
x_train, x_dev = x_shuffled[:dev_sample_index], x_shuffled[shuffle_indices:]
y_train, y_dev = y_shuffled[:dev_sample_index], y_shuffled[shuffle_indices:]
    ...
    if __name__ == '__main__':
        with sess.as_default():
            cnn = TextCNN(
                ...
                vacab_size=len(vocab_processor.vocabulary_),
                embedding_size=FLAGS.embedding_dim,
                filter_sizes=list(map(int, FLAGS.filter_sizes.split(","))),
                num_filters=FLAGS.num_filters,
                r2_reg_lambda=FLAGS.l2_reg_lambda)

            # Define Training procedure
            global_step = tf.Varialbe(0, name="global_step", trainable=False)
            ...
            for g, v in grads_and_vars:
                if g is not None:
                    ...
                    sparsity_summary = tf.scalar_summray("{}/grad/sparsiry".format(v.name), tf.nn.zero_fraction(g))
                    grads_summaries.append(grad_hist_summary)
                    grads_summaries.append(sparsity_summary)
            grads_summaries_merged = tf.merge_summary(grads_summaries)
            ...
            # Train Summaies
            train_summary_op = tf.merge_summary([loss_summary, acc_summary, grads_summaries_merged])
            ...
            def train_step(x_batch, y_batch):
                ...
                _, step, summaries, loss, accuracy = sess.run(
                    ...
                    feed_dict)
                time_str = datetime.datetime.now().iosformat()
            ...
            # Training loop. For each batches..
            for batch in batches:
                ...
                if current_step % FLAGS.checkpoint_every == 0:
                    ...
                    print("Saved model checkpoint to {}\m".format(path))